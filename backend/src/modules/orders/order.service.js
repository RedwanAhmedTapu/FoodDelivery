...
const STATUS_NOTIFICATION_MAP = {
  ...
  REJECTED: {
    ...
    message: "Your order was rejected by the store.",
  },
  PREPARING: {
    type: "FOOD_PREPARING",
    title: "Preparing your food",
    message: "The store has started preparing your order.",
  },
  READY_FOR_PICKUP: {
    type: "FOOD_READY",
    title: "Order ready",
...
async function createOrder(customerId, payload) {
  ...
  const subtotalCheck = cart.items.reduce(
    (sum, i) =>
      sum +
      (i.unitPrice + i.addons.reduce((s, a) => s + a.price, 0)) * i.quantity,
    0,
  );
  ...
  const order = await runTransaction(async (session) => {
      ...
      [
        {
          ...
          deliveryLocation: {
            type: "Point",
            coordinates: payload.deliveryCoordinates,
          },
   ...
   emitToUser(store.ownerId, "order:new", {
      orderId: order._id,
...
async function getOrderById(id) {
  const order = await Order.findById(id).populate(
    "storeId customerId deliveryBoyId",
  );
...
function assertParticipant(order, user) {
  ...
  if (role === "CUSTOMER" && toIdString(order.customerId) === _id.toString())
    return;
  if (role === "SHOP_OWNER" && toIdString(order.ownerId) === _id.toString())
    return;
...
async function updateOrderStatus(orderId, actor, newStatus) {
  ...
  if (
    ...
    order.ownerId.toString() !== actor._id.toString()
  ) {
...
async function verifyDeliveryOtp(orderId, actor, otp) {
  ...
  if (order.orderStatus !== "ON_THE_WAY") {
    throw ApiError.badRequest(
      "Order must be on the way before delivery can be confirmed",
    );
  }
  if (order.deliveryOtpAttempts >= 5) {
    throw ApiError.badRequest(
      "Too many incorrect attempts. Ask the customer to re-check their OTP.",
    );
  }
...
async function onOrderDelivered(order) {
  const settings =
    await require("../platformSettings/platformSettings.model").getSettings();
  const earnedPoints = Math.floor(
    (order.subtotal / 100) * settings.pointsRules.earnRatePer100,
  );
  ...
  await Promise.all(
    order.items.map((item) => foodService.adjustStock(item.foodId, 0)),
  ); // stock already adjusted at creation
  ...
  if (order.referralCode) {
    try {
      ...
      await referralService.rewardQualifyingOrder(
        order.customerId,
        referral._id,
        order._id,
      );
  ...
  try {
    ...
    if (order.deliveryBoyId) {
      const rider = await DeliveryBoy.findById(order.deliveryBoyId).select(
        "userId",
      );
    ...
    // eslint-disable-next-line no-console
    console.error(
      `[wallet] Failed to settle earnings for order ${order._id}:`,
      err.message,
    );
  }
}
...
async function listMyOrders(customerId, query) {
  ...
  return paginate(Order, filter, pagination, {
    populate: [{ path: "storeId", select: "name logo" }],
  });
}
